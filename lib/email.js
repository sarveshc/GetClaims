/**
 * lib/email.js — Transactional Email via Resend
 *
 * Requires one environment variable:
 *   RESEND_API_KEY   — API key from resend.com
 *   ADMIN_EMAIL      — inbox that receives new case notifications
 *   FROM_NAME        — display name (default: "GetClaims")
 *
 * From address: "GetClaims <support@getclaims.in>"
 * Domain getclaims.in must be verified in your Resend dashboard.
 */
...
function FROM() {
  ...
  return `${name} <support@getclaims.in>`;
}
              ...
              <tr>
                <td style="padding:18px 24px">
                  ...
                  <p style="margin:0;color:#6b7280;font-size:12px;line-height:1.6">
                    📧 <a href="mailto:support@getclaims.in" style="color:#0d6efd;text-decoration:none">support@getclaims.in</a> &nbsp;|&nbsp;
                    📞 +91 80468-10500
                  </p>
                </td>
              </tr>
        ...
        <tr>
          <td style="padding:28px 32px">
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e5e7eb;border-radius:8px;overflow:hidden;margin-bottom:20px">
              ...
              <tr><td style="padding:10px 14px;color:#6b7280;font-size:12px;border-bottom:1px solid #e5e7eb;background:#f9fafb">Nature of Complaint</td>
                <td style="padding:10px 14px;border-bottom:1px solid #e5e7eb">
...
export async function sendAcknowledgementEmail(submission) {
  ...
  if (!resend) {
    console.warn("RESEND_API_KEY not set — skipping acknowledgement email");
  ...
  try {
    ...
    console.log("Acknowledgement email sent:", data.id);
