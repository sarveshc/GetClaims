...
function getTransporter() {
  ...
  if (!_transporter) {
    ...
    _transporter = nodemailer.createTransport({
      ...
      // Short timeouts are critical on Vercel (10s function limit).
      // Without these, nodemailer defaults to 2-minute connection timeout
      // which causes the serverless function to be killed mid-flight.
      connectionTimeout: 5000,  // 5s to establish TCP connection
      greetingTimeout:   5000,  // 5s to receive SMTP greeting
      socketTimeout:     6000,  // 6s of inactivity before giving up
      tls: {
