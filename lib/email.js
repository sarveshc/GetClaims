...
// Lazy initialisation — only create Resend instance when a key exists.
// Returns null safely during build time when env vars are not available.
let _resend = null;
function getResend() {
...
function FROM() {
  return `${process.env.FROM_NAME || "GetClaims"} <${process.env.FROM_EMAIL || "onboarding@resend.dev"}>`;
}
              ...
              <tr>
                <td style="padding:18px 24px">
                  ...
                  <p style="margin:0;color:#6b7280;font-size:12px;line-height:1.6">
                    📞 Call / WhatsApp: <a href="tel:+91XXXXXXXXXX" style="color:#0d6efd;text-decoration:none">+91 XXXXXXXXXX</a><br>
        ...
        <tr>
          <td style="background:#1a1a2e;padding:20px 40px;text-align:center">
            <p style="margin:0;color:#9ca3af;font-size:11px;line-height:1.6">
              © ${new Date().getFullYear()} GetClaims. All rights reserved.<br>
      ...
      <table width="640" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08)">
        ...
        <tr>
          <td style="padding:28px 32px 0">
            ...
            <!-- Personal Details -->
            <h3 style="margin:0 0 12px;color:#1a1a2e;font-size:13px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:#6b7280">Personal Details</h3>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e5e7eb;border-radius:8px;overflow:hidden;margin-bottom:20px">
              ...
              <tr style="background:#f9fafb">
                ...
                <td style="padding:10px 14px;color:#111827;font-size:13px;font-weight:700;color:#0d6efd">${formatAmount(claimAmount)}</td>
              </tr>
            </table>
...
export async function sendAcknowledgementEmail(submission) {
  ...
  if (!resend) {
    console.warn("RESEND_API_KEY not configured — skipping acknowledgement email");
  ...
  try {
    ...
    if (error) console.error("Acknowledgement email error:", error);
    return { success: !error, data };
  } catch (err) {
    console.error("sendAcknowledgementEmail threw:", err);
    return { success: false };
  }
}
...
export async function sendAdminNotificationEmail(submission) {
  ...
  try {
    ...
    if (error) console.error("Admin notification email error:", error);
    return { success: !error, data };
  } catch (err) {
    console.error("sendAdminNotificationEmail threw:", err);
    return { success: false };
  }
}
